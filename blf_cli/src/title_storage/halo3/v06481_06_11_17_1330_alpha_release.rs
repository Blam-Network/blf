...
use blf_lib::blf::versions::halo3::v06481_06_11_17_1330_alpha_release::{s_blf_chunk_hopper_description_table, s_blf_chunk_matchmaking_tips, s_blf_chunk_message_of_the_day, s_blf_chunk_network_configuration, s_blf_chunk_packed_game_variant, s_blf_chunk_game_set, s_blf_chunk_author};
...
use blf_lib::blf::versions::halo3::v06481_06_11_17_1330_alpha_release::s_blf_chunk_hopper_configuration_table;
use blf_lib::blf::versions::halo3::v06481_06_11_17_1330_alpha_release::{s_blf_chunk_banhammer_messages, s_blf_chunk_online_file_manifest};
...
title_converter! (
    #[Title("Halo 3")]
    #[Build("06481.06.11.17.1330.alpha_release")]
    pub struct v06481_06_11_17_1330_alpha_release {}
);
...
mod title_storage_output {
    ...
    use blf_lib::blf::versions::halo3::v06481_06_11_17_1330_alpha_release::{s_blf_chunk_game_set, s_blf_chunk_hopper_configuration_table, s_blf_chunk_hopper_description_table, s_blf_chunk_network_configuration, s_blf_chunk_online_file_manifest, s_blf_chunk_packed_game_variant};
    ...
    pub const matchmaking_tips_file_name: &str = "matchmaking_tips.txt";
    pub fn matchmaking_tips_file_path(hoppers_path: &String) -> String {
        build_path!(
            hoppers_path,
    ...
    pub const motd_file_name: &str = "alpha_motd.txt";
    pub fn motd_file_path(hoppers_path: &String) -> String {
        build_path!(
            hoppers_path,
            motd_file_name
        )
    ...
    pub fn hopper_descriptions_file_path(hoppers_path: &String) -> String {
        build_path!(
            hoppers_path,
...
mod title_storage_config {
    ...
    use blf_lib::blf::versions::halo3::v06481_06_11_17_1330_alpha_release::{c_hopper_configuration, s_game_hopper_custom_category};
    ...
    pub fn matchmaking_tips_file_path(config_folder: &String) -> String {
        build_path!(
            config_folder,
            format!("matchmaking_tips.txt")
        )
    }

    pub fn motd_file_path(config_folder: &String) -> String {
        build_path!(
            config_folder,
            format!("motd.txt")
        )
    ...
    pub fn network_configuration_file_path(hopper_folder: &String) -> String {
        build_path!(
            hopper_folder,
            network_configuration_file_name()
        )
...
impl TitleConverter for v06481_06_11_17_1330_alpha_release {
    fn build_blfs(&mut self, config_path: &String, blfs_path: &String) {
        ...
        for hopper_directory in hopper_directories {
            let result = || -> Result<(), Box<dyn Error>> {
                ...
                Self::build_blf_motd(&hopper_config_path, &hopper_blfs_path)?;
    ...
    fn build_config(&mut self, blfs_path: &String, config_path: &String) {
        ...
        for hopper_directory in hopper_directories {
            let result = || -> Result<(), Box<dyn Error>> {
                ...
                Self::build_config_motd(&hoppers_blf_path, &hoppers_config_path)?;
...
impl v06481_06_11_17_1330_alpha_release {
    fn build_config_matchmaking_tips(hoppers_blf_path: &String, hoppers_config_path: &String) -> BLFLibResult {
        ...
        fs::copy(blf_file_path, title_storage_config::matchmaking_tips_file_path(&hoppers_config_path))?;

        やった!(task)
    }

    fn build_config_motd(hoppers_blf_path: &String, hoppers_config_path: &String) -> BLFLibResult {
        let mut task = console_task::start("Converting MOTD");
        ...
        fs::copy(blf_file_path, title_storage_config::motd_file_path(&hoppers_config_path))?;
    ...
    ) -> BLFLibResult<HashMap<u16, String>>
    {
        ...
        if !check_file_exists(&hopper_descriptions_path) {
            ...
            return Ok(hoppers_description_map);
        }
    ...
    fn build_config_hoppers(hoppers_blfs_path: &String, hoppers_config_path: &String) -> BLFLibResult {
        ...
        for hopper_configuration in hopper_configurations {
            let mut hopper_configuration_json = title_storage_config::matchmaking_hopper {
                description: hopper_descriptions.get(&hopper_configuration.hopper_identifier).unwrap_or(&mut String::new()).clone(),
        ...
        for category_configuration in category_configurations {
            let mut category_configuration_and_description = title_storage_config::matchmaking_hopper_category_configuration_and_descriptions {
                ...
                if hopper_descriptions.contains_key(&category_configuration_and_description.configuration.category_identifier)
                {
                    category_configuration_and_description.description = hopper_descriptions.get(&category_configuration_and_description.configuration.category_identifier).unwrap().clone();

                }
                else {
                    category_configuration_and_description.description = String::new();
                }
    ...
    fn build_blf_matchmaking_tips(hoppers_config_folder: &String, hoppers_blf_folder: &String) -> BLFLibResult {
        ...
        let config_path = title_storage_config::matchmaking_tips_file_path(hoppers_config_folder);
        let output_path = title_storage_output::matchmaking_tips_file_path(hoppers_blf_folder);

        if !exists(&config_path)? {
            task.add_warning(format!("Matchmaking tips are missing."));
            return やった!(task)
        }

        create_parent_folders(&output_path)?;
        fs::copy(config_path, output_path)?;

        やった!(task)
    }

    fn build_blf_motd(hoppers_config_folder: &String, hoppers_blf_folder: &String) -> BLFLibResult {
        let mut task = console_task::start("Building MOTD");

        let config_path = title_storage_config::motd_file_path(hoppers_config_folder);
        let output_path = title_storage_output::motd_file_path(hoppers_blf_folder);

        if !exists(&config_path)? {
            task.add_warning("MOTD is missing.");
            やった!(task)
        }
    ...
    fn build_blf_map_manifest(hoppers_config_path: &String, hoppers_blf_path: &String) -> BLFLibResult
    {
        ...
        BlfFileBuilder::new()
            ...
            .add_chunk(s_blf_chunk_author::for_build::<v06481_06_11_17_1330_alpha_release>())
        ...
        for (hopper_id, game_set_config) in active_game_sets {
            ...
            for row in game_set_config.entries.iter() {
                gset.add_entry(s_blf_chunk_game_set_entry {
                    map_id: row.map_id,
                    map_variant_file_name: StaticString::default(),
    ...
    ) -> BLFLibResult
    {
        ...
        for (hopper_identifier, hopper_configuration_json) in &hopper_configuration_jsons {
            let hopper_config = hopper_configuration_json.configuration.clone();
        ...
        // Initialize hopper_descriptions
        let mut mhdf = s_blf_chunk_hopper_description_table::default();

        for (hopper_identifier, hopper_configuration_json) in &hopper_configuration_jsons {
            mhdf.add_description((
                *hopper_identifier,
                &hopper_configuration_json.description.to_string()
            ))?;

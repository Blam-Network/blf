...
mod title_storage_config {
    ...
    pub fn network_configuration_file_path(hoppers_root: &String) -> String {
        build_path!(
            hoppers_root,
            network_configuration_file_name()
        )
...
impl TitleConverter for v08172_07_03_08_2240_delta {
    fn build_blfs(&mut self, config_path: &String, blfs_path: &String) {
        ...
        Self::build_blf_network_configuration(&config_path, &blfs_path).unwrap_or_else(|err| {
            println!("{}", err);
        });
        ...
        Self::build_blf_manifest(&blfs_path).unwrap_or_else(|err| {
            println!("{}", err);
        });
    ...
    fn build_config(&mut self, blfs_path: &String, config_path: &String) {
        ...
        Self::build_config_network_configuration(&blfs_path, &config_path).unwrap_or_else(|err| {
            println!("{}", err);
        });
...
impl v08172_07_03_08_2240_delta {
    ...
    fn build_blf_manifest(
        hoppers_blfs_root: &String,
    ) -> BLFLibResult {
        let mut task = console_task::start("Building Manifest File");

        let mut manifest_chunk = s_blf_chunk_online_file_manifest::default();
        let hopper_directory_name = "default_hoppers";
        let default_hoppers_path = build_path!(
            hoppers_blfs_root,
            hopper_directory_name
        );
        ...
        let mut add_hash_if_file_exists = |manifest_path: String, file_path: String| -> BLFLibResult {
            if exists(&file_path)? {
                ...
                task.add_warning(format!("{manifest_path} was not added to the manifest as its file was not found."))
            }
            Ok(())
        };
        ...
        add_hash_if_file_exists(
            ...
            title_storage_output::matchmaking_hopper_file_path(&default_hoppers_path)
        )?;
        ...
        add_hash_if_file_exists(
            ...
            title_storage_output::network_configuration_file_path(&default_hoppers_path)
        )?;
        ...
        add_hash_if_file_exists(
            ...
            title_storage_output::rsa_manifest_file_path(&default_hoppers_path)
        )?;
        ...
        add_hash_if_file_exists(
            ...
            title_storage_output::hopper_descriptions_file_path(&default_hoppers_path)
        )?;
        ...
        for language_code in k_language_suffixes {
            add_hash_if_file_exists(
                ...
                title_storage_output::banhammer_messages_file_path(&default_hoppers_path, language_code)
            )?;
            ...
            add_hash_if_file_exists(
                ...
                title_storage_output::matchmaking_tips_file_path(&default_hoppers_path, language_code)
            )?;
        }

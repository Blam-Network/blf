...
const GEA0 = new URL("../tests/fixtures/haloreach_gea0_map.blf", import.meta.url);
...
function mpvrPayload(buf) {
  for (let o = 0; o + 12 <= buf.length; o++) {
    if (buf[o] === 0x6d && buf[o + 1] === 0x70 && buf[o + 2] === 0x76 && buf[o + 3] === 0x72) {
      const cs = new DataView(buf.buffer, buf.byteOffset).getUint32(o + 4, false);
...
const vl = new DataView(payload.buffer, payload.byteOffset + 24).getUint32(0, false);
...
if (firstDiff < 0) console.log("byte-identical gametype");

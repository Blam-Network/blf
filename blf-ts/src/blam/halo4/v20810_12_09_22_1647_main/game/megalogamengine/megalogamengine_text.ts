...
/** Halo 4 MaxStrings=148 → biased write_index<148> (8 bits). */
const k_dynamic_string_index_max = 148;
const k_dynamic_string_index_bits = 8;

function tokenCountBitWidth(maxTokens: number): number {
  // Encode token count in [0, maxTokens] — same as engine required_bits(max+1).
  return Math.max(1, Math.ceil(Math.log2(maxTokens + 1)));
}

/**
 * Halo 4 `c_dynamic_string_base<N>` — `maxTokens` is the template parameter N
 * (token-count bit width is ceil(log2(N+1))).
 */
export class c_dynamic_string {
  ...
  decode(bitstream: c_bitstream_reader): void {
    this.m_string_index = bitstream.read_index(
      "string-index",
      k_dynamic_string_index_max,
      k_dynamic_string_index_bits
    );
    const tokenCount = bitstream.read_integer(
      "token-count",
      tokenCountBitWidth(this.maxTokens)
    );
    this.m_tokens = [];

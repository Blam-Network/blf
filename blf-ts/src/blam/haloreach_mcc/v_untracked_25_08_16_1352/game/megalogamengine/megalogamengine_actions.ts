...
/**
 * Matches `c_flags<e_player_purchase_mode, …, 5>` in managedmegalo.
 * Script form: `{alive|dead|both} {weapons|equipment|vehicles|all}`.
 * There is no dead+vehicles combination (that AND clears the mask).
 */
export class e_player_purchase_mode_flags {
  @AutoMap(() => Boolean)
  alive_weapons = false;
  @AutoMap(() => Boolean)
  alive_equipment = false;
  @AutoMap(() => Boolean)
  alive_vehicles = false;
  @AutoMap(() => Boolean)
  dead_weapons = false;
  @AutoMap(() => Boolean)
  dead_equipment = false;
  to_raw(): number {
    return (
      (this.alive_weapons ? 1 : 0) |
      (this.alive_equipment ? 1 << 1 : 0) |
      (this.alive_vehicles ? 1 << 2 : 0) |
      (this.dead_weapons ? 1 << 3 : 0) |
      (this.dead_equipment ? 1 << 4 : 0)
    );
  }
  static from_raw(raw: number): e_player_purchase_mode_flags {
    const flags = new e_player_purchase_mode_flags();
    flags.alive_weapons = (raw & 1) !== 0;
    flags.alive_equipment = (raw & (1 << 1)) !== 0;
    flags.alive_vehicles = (raw & (1 << 2)) !== 0;
    flags.dead_weapons = (raw & (1 << 3)) !== 0;
    flags.dead_equipment = (raw & (1 << 4)) !== 0;
    return flags;
  }
}
...
export class s_action_player_enable_purchases_parameters {
  ...
  @AutoMap(() => c_custom_variable_reference)
  m_variable = new c_custom_variable_reference();
  @AutoMap(() => e_player_purchase_mode_flags)
  m_mode = new e_player_purchase_mode_flags();
  decode(bitstream: c_bitstream_reader): void {
    this.m_player.decode(bitstream);
    this.m_variable.decode(bitstream);
    this.m_mode = e_player_purchase_mode_flags.from_raw(
  ...
  encode(bitstream: c_bitstream_writer): void {
    this.m_player.encode(bitstream);
    this.m_variable.encode(bitstream);
    bitstream.write_integer(this.m_mode.to_raw(), 5);
  }
}
...
export class c_action {
  ...
  executable_pregame(): boolean {
    return (
      ...
      this.m_type === e_action_type.begin
    );
  }
}

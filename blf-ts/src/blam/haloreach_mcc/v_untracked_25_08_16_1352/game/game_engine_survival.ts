...
export class c_game_engine_survival_variant {
  ...
  @AutoMap(() => e_survival_variant_flags)
  m_variant_flags = new e_survival_variant_flags();
  /** 3 bits; when >= 2, `m_additional_flags` is present on the wire. */
  @AutoMap(() => Number)
  ...
  /** 8 bits when `m_encoding_version` >= 2 (e.g. alpha_sync_slayer). */
  @AutoMap(() => Number)
  ...
  decode(bitstream: c_bitstream_reader): void {
    ...
    this.m_encoding_version = bitstream.read_integer("encoding-version", 3);
    ...
    if (this.m_encoding_version >= 2) {
      this.m_additional_flags = bitstream.read_integer("additional-flags", 8);
    }
  }
  encode(bitstream: c_bitstream_writer): void {
    this.m_base_variant.encode(bitstream);
    bitstream.write_integer(this.m_variant_flags.to_raw(), 5);
    bitstream.write_integer(this.m_encoding_version, 3);
    ...
    if (this.m_encoding_version >= 2) {
      bitstream.write_integer(this.m_additional_flags, 8);
    }
  }
}

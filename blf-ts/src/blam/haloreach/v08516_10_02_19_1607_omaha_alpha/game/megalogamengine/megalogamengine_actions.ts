...
/** Matches `e_action_type` in blf_lib omaha_alpha `megalogamengine_actions.rs`. */
...
/**
 * Create-object flag helpers for compilers.
 * Alpha wire only stores bit 0 (`never_garbage_collect`) as a 1-bit integer;
 * other fields are ignored on encode.
 */
...
import { c_dynamic_string } from "./megalogamengine_text";
...
export class s_action_set_score_parameters {
  ...
  decode(bitstream: c_bitstream_reader): void {
    ...
    this.m_operation = bitstream.read_integer(
      "operation",
      3
    ) as e_math_operation;
  ...
  encode(bitstream: c_bitstream_writer): void {
    ...
    bitstream.write_integer(this.m_operation, 3);
...
export class s_action_create_object_parameters {
  ...
  @AutoMap(() => Number)
  m_filter_index = 0;
  /** Alpha: 1 bit (Rust omaha_alpha). Later builds use a 3-bit flag set. */
  @AutoMap(() => Number)
  m_flags = 0;
  decode(bitstream: c_bitstream_reader): void {
    ...
    this.m_flags = bitstream.read_integer("flags", 1);
  }
  encode(bitstream: c_bitstream_writer): void {
...
export class s_action_set_progress_bar_parameters {
  ...
  decode(bitstream: c_bitstream_reader): void {
    this.m_object.decode(bitstream);
    this.m_team_filter = bitstream.read_integer("team-filter", 2);
    if (this.m_team_filter !== 0) {
      this.m_timer_index = bitstream.read_index("timer-index", 4, 2);
    }
  }
  encode(bitstream: c_bitstream_writer): void {
    this.m_object.encode(bitstream);
    bitstream.write_integer(this.m_team_filter, 2);
    if (this.m_team_filter !== 0) {
      bitstream.write_index(this.m_timer_index, 4, 2);
    }
  }
...
export class s_action_player_adjust_money_parameters {
  ...
  decode(bitstream: c_bitstream_reader): void {
    this.m_player.decode(bitstream);
    this.m_math_operation = bitstream.read_integer(
      "math-operation",
      3
    ) as e_math_operation;
    this.m_variable.decode(bitstream);
  ...
  encode(bitstream: c_bitstream_writer): void {
    this.m_player.encode(bitstream);
    bitstream.write_integer(this.m_math_operation, 3);
    this.m_variable.encode(bitstream);
  }
}
...
export class s_action_object_set_minimap_visibility_parameters {
  @AutoMap(() => c_object_reference)
  m_object = new c_object_reference();
  @AutoMap(() => Number)
  m_value = 0;
  ...
  decode(bitstream: c_bitstream_reader): void {
    this.m_object.decode(bitstream);
    this.m_value = bitstream.read_integer("value", 2);
  }

  encode(bitstream: c_bitstream_writer): void {
    this.m_object.encode(bitstream);
    bitstream.write_integer(this.m_value, 2);
  }
}

export class s_action_object_set_minimap_priority_parameters {
  @AutoMap(() => c_object_reference)
  m_object = new c_object_reference();
  @AutoMap(() => Number)
  m_value = 0;
...
export class s_action_object_set_minimap_icon_parameters {
  @AutoMap(() => c_object_reference)
  m_object = new c_object_reference();
  @AutoMap(() => Number)
  m_icon_index = 0;
  ...
  decode(bitstream: c_bitstream_reader): void {
    this.m_object.decode(bitstream);
    this.m_icon_index = bitstream.read_integer("icon-index", 4);
  }

  encode(bitstream: c_bitstream_writer): void {
    this.m_object.encode(bitstream);
    bitstream.write_integer(this.m_icon_index, 4);
  }
}
...
export class s_action_set_loadout_palette_parameters {
  ...
  decode(bitstream: c_bitstream_reader): void {
    this.m_target.decode(bitstream);
    this.m_palette_index = bitstream.read_integer("palette-index", 8);
  }
  encode(bitstream: c_bitstream_writer): void {
    this.m_target.encode(bitstream);
    bitstream.write_integer(this.m_palette_index, 8);
  }
}
...
export class s_action_object_set_scale_parameters {
  ...
  decode(bitstream: c_bitstream_reader): void {
    this.m_object.decode(bitstream);
    // Alpha uses Halo3 quantized-real (exact_midpoint); endpoints unused.
    this.m_scale = bitstream.read_quantized_real(0, 5, 7, true, false);
  }
  encode(bitstream: c_bitstream_writer): void {
    this.m_object.encode(bitstream);
    bitstream.write_quantized_real(this.m_scale, 0, 5, 7, true, false);
  }
}
...
export class s_action_player_set_fireteam_tier_parameters {
  @AutoMap(() => c_player_reference)
  m_player: c_player_reference = new c_player_reference();
  @AutoMap(() => c_custom_variable_reference)
  m_variable: c_custom_variable_reference = new c_custom_variable_reference();
...
export class s_action_give_weapon_parameters {
  @AutoMap(() => c_object_type_reference)
  m_object_type: c_object_type_reference = new c_object_type_reference();
  @AutoMap(() => c_player_reference)
  m_player: c_player_reference = new c_player_reference();
  @AutoMap(() => Number)
  m_flag = 0;
  decode(bitstream: c_bitstream_reader): void {
    this.m_object_type.decode(bitstream);
    this.m_player.decode(bitstream);
    this.m_flag = bitstream.read_integer("flag", 1);
  }
  encode(bitstream: c_bitstream_writer): void {
    this.m_object_type.encode(bitstream);
    this.m_player.encode(bitstream);
    bitstream.write_integer(this.m_flag, 1);
  }
}
export class s_action_set_loadout_parameters {
  @AutoMap(() => s_team_or_player_target)
  m_target = new s_team_or_player_target();
  @AutoMap(() => Number)
  m_loadout_index = 0;
  decode(bitstream: c_bitstream_reader): void {
    this.m_target.decode(bitstream);
    this.m_loadout_index = bitstream.read_integer("loadout-index", 8);
  }
  encode(bitstream: c_bitstream_writer): void {
    this.m_target.encode(bitstream);
    bitstream.write_integer(this.m_loadout_index, 8);
  }
}
export class c_action {
  ...
  @AutoMap(() => s_action_object_set_minimap_visibility_parameters)
  m_object_set_minimap_visibility_parameters?: s_action_object_set_minimap_visibility_parameters;
  @AutoMap(() => s_action_object_set_minimap_priority_parameters)
  m_object_set_minimap_priority_parameters?: s_action_object_set_minimap_priority_parameters;
  @AutoMap(() => s_action_object_set_minimap_icon_parameters)
  m_object_set_minimap_icon_parameters?: s_action_object_set_minimap_icon_parameters;
  ...
  @AutoMap(() => s_action_player_set_fireteam_tier_parameters)
  m_player_set_fireteam_tier_parameters?: s_action_player_set_fireteam_tier_parameters;
  ...
  @AutoMap(() => s_action_give_weapon_parameters)
  m_give_weapon_parameters?: s_action_give_weapon_parameters;
  ...
  @AutoMap(() => s_action_set_loadout_parameters)
  m_set_loadout_parameters?: s_action_set_loadout_parameters;
  ...
  decode(bitstream: c_bitstream_reader): void {
    ...
    switch (this.m_type) {
      ...
      case e_action_type.object_set_minimap_visibility: {
        const object_set_minimap_visibility_parameters =
          new s_action_object_set_minimap_visibility_parameters();
        object_set_minimap_visibility_parameters.decode(bitstream);
        this.m_object_set_minimap_visibility_parameters =
          object_set_minimap_visibility_parameters;
        break;
      }
      case e_action_type.object_set_minimap_priority: {
        const object_set_minimap_priority_parameters =
          new s_action_object_set_minimap_priority_parameters();
        object_set_minimap_priority_parameters.decode(bitstream);
        this.m_object_set_minimap_priority_parameters =
          object_set_minimap_priority_parameters;
        break;
      }
      case e_action_type.object_set_minimap_icon: {
        const object_set_minimap_icon_parameters =
          new s_action_object_set_minimap_icon_parameters();
        object_set_minimap_icon_parameters.decode(bitstream);
        this.m_object_set_minimap_icon_parameters =
          object_set_minimap_icon_parameters;
        break;
      }
      ...
      case e_action_type.player_set_fireteam_tier: {
        const player_set_fireteam_tier_parameters =
          new s_action_player_set_fireteam_tier_parameters();
        player_set_fireteam_tier_parameters.decode(bitstream);
        this.m_player_set_fireteam_tier_parameters =
          player_set_fireteam_tier_parameters;
        break;
      }
      ...
      case e_action_type.give_weapon: {
        const give_weapon_parameters = new s_action_give_weapon_parameters();
        give_weapon_parameters.decode(bitstream);
        this.m_give_weapon_parameters = give_weapon_parameters;
        break;
      }
      ...
      case e_action_type.set_loadout: {
        const set_loadout_parameters = new s_action_set_loadout_parameters();
        set_loadout_parameters.decode(bitstream);
        this.m_set_loadout_parameters = set_loadout_parameters;
        break;
      }
  ...
  encode(bitstream: c_bitstream_writer): void {
    ...
    switch (this.m_type) {
      ...
      case e_action_type.player_set_objective:
        ...
        break;
      case e_action_type.object_set_minimap_visibility:
        this.m_object_set_minimap_visibility_parameters?.encode(bitstream);
        break;
      case e_action_type.object_set_minimap_priority:
        this.m_object_set_minimap_priority_parameters?.encode(bitstream);
        break;
      case e_action_type.object_set_minimap_icon:
        this.m_object_set_minimap_icon_parameters?.encode(bitstream);
        break;
      case e_action_type.team_set_coop_spawning:
      ...
      case e_action_type.player_set_requisition_palette:
        ...
        break;
      case e_action_type.player_set_fireteam_tier:
        this.m_player_set_fireteam_tier_parameters?.encode(bitstream);
        break;
      case e_action_type.device_set_power:
      ...
      case e_action_type.device_get_position:
        ...
        break;
      case e_action_type.give_weapon:
        this.m_give_weapon_parameters?.encode(bitstream);
        break;
      case e_action_type.adjust_grenades:
      ...
      case e_action_type.submit_incident_with_custom_value:
        ...
        break;
      case e_action_type.set_loadout:
        this.m_set_loadout_parameters?.encode(bitstream);
        break;
      case e_action_type.set_loadout_palette:

...
/**
 * Omaha Alpha player traits bitstream (matches blf_lib omaha_alpha
 * `game_engine_player_traits.rs`). Values are raw integers — stock mglos use
 * option indices outside the later TU1 enum ranges.
 */
...
export class c_player_traits {
  ...
  decode(bitstream: c_bitstream_reader): void {
    ...
    if (bitstream.read_bool("player-traits-movement-jump-modifier-changed")) {
      // Halo3-style quantized real (4 bits, no exact endpoints).

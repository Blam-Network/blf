...
import { c_game_engine_respawn_options } from "./game_engine_traits";

/** Matches `c_ai_traits` in blf_lib omaha_alpha `game_engine_survival.rs`. */
...
/** Matches `s_survival_wave_properties` in blf_lib omaha_alpha `game_engine_survival.rs`. */
export class s_survival_wave_properties {
  ...
  decode(bitstream: c_bitstream_reader): void {
    ...
    this.m_duration_seconds = bitstream.read_integer("duration_seconds", 12);
    this.m_wave_squad_advance_type = bitstream.read_integer(
      "wave_squad_advance_type",
      1
    );
    this.m_skull_flags = bitstream.read_integer("skull-flags", 9);
    this.m_wave_squad_count = bitstream.read_integer("wave-squad-count", 4);
    this.m_squads = [];
    for (let i = 0; i < 12; i++) {
      this.m_squads.push(bitstream.read_integer("possible-wave-squad", 8));
    }
  }

  encode(bitstream: c_bitstream_writer): void {
    bitstream.write_integer(this.m_wave_flags, 1);
    bitstream.write_integer(this.m_duration_seconds, 12);
    bitstream.write_integer(this.m_wave_squad_advance_type, 1);
    bitstream.write_integer(this.m_skull_flags, 9);
    bitstream.write_integer(this.m_wave_squad_count, 4);
    for (let i = 0; i < 12; i++) {
      bitstream.write_integer(this.m_squads[i] ?? 0, 8);
    }
  }
}

/** Matches `s_survival_round_properties` in blf_lib omaha_alpha `game_engine_survival.rs`. */
...
/** Matches `c_game_engine_survival_variant` in blf_lib omaha_alpha `game_engine_survival.rs`. */
export class c_game_engine_survival_variant {
  ...
  @AutoMap(() => [Number])
  m_skull_flags: number[] = Array.from({ length: 9 }, () => 0);
  ...
  decode(bitstream: c_bitstream_reader): void {
    ...
    this.m_variant_flags = bitstream.read_integer("m_variant_flags", 3);
    this.m_campaign_difficulty_level = bitstream.read_integer(
      "campaign-difficulty-level",
      3
    );
    this.m_time_limit_seconds = bitstream.read_integer(
      "m_time_limit_seconds",
      11
    );
    this.m_set_count = bitstream.read_integer("set-count", 4);
    ...
    this.m_skull_flags = [];
    for (let i = 0; i < 9; i++) {
      this.m_skull_flags.push(bitstream.read_integer("skull-flags", 9));
    }
  ...
  encode(bitstream: c_bitstream_writer): void {
    ...
    bitstream.write_integer(this.m_variant_flags, 3);
    bitstream.write_integer(this.m_campaign_difficulty_level, 3);
    bitstream.write_integer(this.m_time_limit_seconds, 11);
    bitstream.write_integer(this.m_set_count, 4);
    ...
    for (let i = 0; i < 9; i++) {
      bitstream.write_integer(this.m_skull_flags[i] ?? 0, 9);
    }

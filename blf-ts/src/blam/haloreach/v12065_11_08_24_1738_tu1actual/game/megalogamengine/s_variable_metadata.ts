...
export class s_variable_metadata {
  ...
  decode(bitstream: c_bitstream_reader): void {
    ...
    for (let i = 0; i < player_variable_count; i++) {
      this.m_player_variables.push(
        bitstream.read_integer("network-state", 2),
      );
    }
  ...
  encode(bitstream: c_bitstream_writer): void {
    bitstream.write_integer(this.m_numeric_variables.length, this.numeric_variable_count_bits);
    ...
    bitstream.write_integer(this.m_timer_variables.length, this.timer_variable_count_bits);
    for (const timer_variable of this.m_timer_variables) {
      timer_variable.encode(bitstream);
    }
    bitstream.write_integer(this.m_team_variables.length, this.team_variable_count_bits);
    for (const [team_variable, network_state] of this.m_team_variables) {
      bitstream.write_integer(team_variable, 4);
      bitstream.write_integer(network_state, 2);
    }
    bitstream.write_integer(this.m_player_variables.length, this.player_variable_count_bits);
    for (const network_state of this.m_player_variables) {
      bitstream.write_integer(network_state, 2);
    }
    bitstream.write_integer(this.m_object_variables.length, this.object_variable_count_bits);

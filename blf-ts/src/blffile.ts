...
import { BLFChunk, getBlfChunkMeta } from "./blf_chunk";
...
export function write_blffile(
    ...
    for (const chunk of chunks) {
      if (getBlfChunkMeta(chunk).signature === "_eof") {
        if (Object.getOwnPropertyNames(chunk).includes("file_size") && typeof (chunk as any).file_size === "number") {

...
pub fn simulation_read_position(
    ...
    if bitstream.read_bool()? { // point-in-initial-bounds
        ...
        let mut quantized_point = int32_point3d::default();
        quantized_point.x = bitstream.read_signed_integer(per_axis_bit_counts.x as usize)?;
        quantized_point.y = bitstream.read_signed_integer(per_axis_bit_counts.y as usize)?;
        quantized_point.z = bitstream.read_signed_integer(per_axis_bit_counts.z as usize)?;

        Ok(dequantize_real_point3d_per_axis(&quantized_point, world_bounds, &per_axis_bit_counts, position))
    }
    else {

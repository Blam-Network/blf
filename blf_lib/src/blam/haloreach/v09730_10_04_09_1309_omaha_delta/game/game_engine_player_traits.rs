...
impl c_player_traits {
    pub fn encode(&self, mut bitstream: &mut c_bitstream_writer) -> BLFLibResult {
        ...
        bitstream.write_integer(self.m_movement_traits.m_sprint_setting, 2)?;
        bitstream.write_integer(self.m_movement_traits.m_equipment_usage_setting, 2)?;
        if self.m_movement_traits.m_jump_modifier != -1.0 {
            bitstream.write_bool(true)?;
            bitstream.write_quantized_real(self.m_movement_traits.m_jump_modifier, 0.0, 4.0, 4, false)?;
        } else {
    ...
    pub fn decode(&mut self, mut bitstream: &mut c_bitstream_reader) -> BLFLibResult {
        ...
        self.m_movement_traits.m_sprint_setting = bitstream.read_integer("player-traits-movement-sprint", 2)?;
        self.m_movement_traits.m_equipment_usage_setting = bitstream.read_integer("player-traits-movement-equipment-usage", 2)?;
        if bitstream.read_bool("player-traits-movement-jump-modifier-changed")? {
            self.m_movement_traits.m_jump_modifier = *bitstream.read_quantized_real(0.0, 4.0, 4, false)?;
        } else {

...
impl c_map_variant {
    pub fn encode(&self, bitstream: &mut c_bitstream_writer) -> BLFLibResult {
        ...
        bitstream.write_integer(self.m_number_of_variant_objects, 10)?;
...
pub struct s_variant_object_datum {
    ...
    pub scenario_object_type: i8,
    pub scenario_object_datum_index: u16,
...
impl s_variant_object_datum {
    ...
    pub fn encode(&self, mut bitstream: &mut c_bitstream_writer, world_bounds: &real_rectangle3d) -> BLFLibResult {
        ...
        bitstream.write_integer((self.scenario_object_type + 1) as u32, 4)?;

        if self.scenario_object_type != -1 {
            bitstream.write_integer(self.scenario_object_datum_index, 13)?;
        }

        simulation_write_quantized_position(bitstream, &self.position, 16, false, &world_bounds)?;
        bitstream.write_axes(&self.forward, &self.up)?;

...
use crate::blam::haloreach::release::io::bitstream_reader::c_bitstream_reader_extensions;
...
pub struct c_map_variant {
    ...
    pub m_variant_objects: StaticArray<s_variant_object_datum, 651>,
...
impl c_map_variant {
    ...
    pub fn decode(&mut self, bitstream: &mut c_bitstream_reader) -> BLFLibResult {
        ...
        self.m_built_from_xml = Bool::from(bitstream.read_bool()?);
...
impl s_variant_quota {
    pub fn decode(&mut self, bitstream: &mut c_bitstream_reader) -> BLFLibResult {
        self.minimum_count = bitstream.read_integer(8)? as u8;
        self.maximum_count = bitstream.read_integer(8)? as u8;
        self.placed_on_map = bitstream.read_integer(8)? as u8;
        Ok(())
    }
}
...
impl s_variant_multiplayer_object_properties_definition {
    pub fn decode(&mut self, bitstream: &mut c_bitstream_reader) -> BLFLibResult {
        ...
        self.user_data = bitstream.read_integer(8)? as u8;
        self.spawn_time = bitstream.read_integer(8)? as u8;
        self.cached_type = bitstream.read_integer(5)? as u8;
        self.label_index = bitstream.read_index::<256>(8)? as i8;
        self.placement_flags = bitstream.read_integer(8)? as u8;
        self.team = bitstream.read_integer(4)? as i8 - 1;
...
impl s_variant_object_datum {
    pub fn decode(&mut self, bitstream: &mut c_bitstream_reader, world_bounds: &real_rectangle3d) -> BLFLibResult {
        if bitstream.read_bool()? { // exists
            self.flags = bitstream.read_integer(2)? as u16;
            ...
            self.spawn_relative_to = bitstream.read_integer(10)? as i32 - 1;

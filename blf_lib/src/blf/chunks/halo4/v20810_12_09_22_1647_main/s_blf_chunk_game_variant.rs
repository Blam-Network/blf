...
use blf_lib::blam::halo4::v20810_12_09_22_1647_main::game::game_variant::c_game_variant;
...
/// Gametype storage slot size (bytes). Halo 4 MaxEncodedSize.
pub const variant_storage_capacity: usize = 31744;

/// mpvr body size: hash/header (28) + padded gametype slot.
pub const MPVR_PAYLOAD_SIZE: usize = 28 + variant_storage_capacity;

/// Halo 4 game variant chunk (`mpvr` 54.1).
...
impl BinRead for s_blf_chunk_game_variant {
    ...
    fn read_options<R: Read + Seek>(
        ...
        let _hash: s_network_http_request_hash = bitstream.read_raw(0x14 * 8)?;
        let _unknown04: i16 = bitstream.read_signed_integer("unknown04", 16)?;
        let _unknown06: u16 = bitstream.read_integer("unknown06", 16)?;
        let _variant_length: u32 = bitstream.read_integer("variant-length", 32)?;

        let mut game_variant = c_game_variant::default();
        game_variant.decode(&mut bitstream)?;

        Ok(Self { game_variant })
    }
}
...
impl BinWrite for s_blf_chunk_game_variant {
    ...
    fn write_options<W: Write + Seek>(
        ...
        let mut bitstream_writer = c_bitstream_writer::new(
        ...
        writer.write_all(&vec![0u8; variant_storage_capacity - gametype_data.len()])?;
